...
datasets_with_consommation = [train]
...
for i in 0:(k-1)
    ...
    model = lm(@formula(consommation ~ age + transmission_integrale + transmission_traction + boite + cylindree), data_k_folds)
...
residuals = ŷ_train .- train.consommation
...
for formula in models_formulas
    ...
    println(model)
    ŷ = GLM.predict(model, train)
    local raj_train = compute_raj(X_train, y_train, ŷ)
    local bic_train = compute_bic(X_train, model)

    println("Raj on the training set: $raj_train")

    local rmse_train = sqrt(mean((ŷ .- y_train).^2))
    println("RMSE on the training set: $rmse_train")
    ...
    if bic_train > best_bic
        best_bic = bic_train
        best_model_by_bic = formula
        bic_rmse = rmse_train
    end

    if raj_train > best_rajs
        best_rajs = raj_train
